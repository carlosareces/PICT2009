...
Los objetivos espec\'ificos se listan a continuaci\'on:

\begin{itemize}
    \item[$\triangleright$] Identificar y delimitar las \'areas de trabajo pertinentes, seg\'un el estado del arte. 
    \item[$\triangleright$] Proponer distintos y novedosos algoritmos de interacci\'on.
    \item[$\triangleright$] Proponer distintos algoritmos de inferencia, estudiando la utilizaci\'on de distintas metodolog\'ias (por caso, distintos tipos de l\'ogias o representaciones del conocimiento).
    \item[$\triangleright$] Proponer una metodolog\'ia de trabajo que permita la conjunci\'on de los puntos anteriores en un prototipo de tutor virtual.
    \item[$\triangleright$] Estudiar y proponer una metodolog\'ia para la aplicaci\'on de t\'ecnicas evaluativas de otros dominios del PLN (en particular de la TA) al dominio de los sistemas de di\'alogos.
    \item[$\triangleright$] Proponer un modelo de evaluaci\'on basada en el contexto para la evaluaci\'on de prototipos por usuarios reales.
    \item[$\triangleright$] Diseñar y ejecutar planes de evaluaciones de acuerdo al punto anterior, que permitan identificar las debilidades del sistema para luego mejorar en fases subsiguientes.
...
A continuaci\'on se detallan el plan de trabajo propuesto para llevar a cabo los objetivos planteados ut supra: en la Subsecci\'on 5.1 se presentan las tareas planificadas y luego en la Subsecci\'on 5.2 se detalla un posible aplicaci\'on del sistema a desarrollar.
...
\begin{enumerate}
    \item Prospecci\'on de metodolog\'ias existentes para el desarrollo de sistemas de di\'alogo a utilizarse en entornos 3D, enfatizando la misma en los algoritmos de interacii\'on e inferencia propuestos.
\begin{enumerate}
    \item Relevamiento bibliogr\'afico
    \item Estudio del material bibliogr\'afico
    \item Estudio de la plataforma GIVE
\end{enumerate}
\item Propuesta y relizaci\'on de las componentes que conformar\'ian el m\'odulo encargado de la generaci\'on de instrucciones y de la interpretaci\'on de devoluciones por el usuario, en base al punto 1.
\begin{enumerate}
\item Recopilaci\'on de un corpus de interacci\'on humano-humano (uni y bidireccional) que contendr\'a la interacci\'on lig\"u\'istica alineada con las acciones realizadas en un mundo virtual.  Este corpus ser\'a utilizado para guiar el dise\~no de las pol\'iticas de interacci\'on a implementar. Trabajos relacionados \citep{HCRC-93, byron-06}. \fixme{arreglar este item}
...
\item Propuesta y prototipado de un sistema de di\'alogo con las capacidades mencionadas en la Secci\'on 4 y que incluya los m\'odulos disponibles del punto anterior (punto 2).
\begin{enumerate}
...
Por otro lado, dado que se usar\'a la plataforma GIVE, se planea la participaci\'on en el evento asociado (ver Secci\'on 6), lo cual servir\'a como una fuente adicional de informaci\'on acerca de los aspectos del sistema a mejorar.
	%\item Caso de estudio: aplicaci\'on
\end{enumerate}

\item Diseminaci\'on de resultados y lecciones aprendidas.
\begin{enumerate}
    \item Documentaci\'on del sistema obtenido, en forma de manuales de usuario y/o del desarrollador que incluyan una descripci\'on detallada de los algoritmos implementados con sus puntos fuertes y d\'ebiles.  
    \item Presentaci\'on de resultados te\'oricos y aplicados a trav\'es de art\'iculos presentados en conferencias locales o internacionales as\'i como tambi\'en en revistas cient\'ificas pertinentes a las \'areas de investigaci\'on afectadas.
% Posibles candidatos son: \textit{International Journal of Speech Technology}, \textit{}\textit{}\textit{} \fixme{Agregar conferencias/journals de interes}
\end{enumerate}
...
\fixme{C/L: esto mas lo comentado podria agregarse al punto 2a-b-c-d para explicar mejor cada tarea?}
...
\MySubSubSection{6. Consideraciones Pr\'acticas}

%El dise\~no de una sistema como el descripto anteriormente presenta desaf\'ios tanto te\'oricos como pr\'acticos.  En lo te\'orico, la necesidad de  guiar la interacci\'on con el usuario (qu\'e decir, cu\'ando y c\'omo, en un contexto dado) motiva la creaci\'on, adaptaci\'on o mejora de heur\'isticas pertinentes y, por otro lado,  la necesidad de describir la situaci\'on actual as\'i como tambi\'en  de indicar c\'omo alterarla para alcanzar un objetivo pre-establecido, incentivan el estudio de diversos m\'etodos para la inferencia eficiente y precisa. La calidad de cada una de estas capacidades afecta la percepci\'on que el usuario tiene del sistema, y por lo tanto, es imperativo aplicar m\'etricas pertinentes que permitan evaluar cada uno de estos aspectos del sistema.  La complejidad del problema te\'orico serefleja, en lo pr\'actico, en el desarrollo de un sistema de m\'ultiples componentes: un generador de lenguaje natural, un componente de planeamiento, un entorno 3D, etc.
